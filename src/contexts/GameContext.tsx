...
import { generateDailyChallenge, formatTimeElapsed } from "@/lib/utils";
...
export const GameProvider: React.FC<{
  ...
  challengeId?: string;
  category?: string;
}> = ({ children, challengeId, category }) => {
  // Initialize game state
  const [gameState, setGameState] = useState<GameState>(() => {
    const dailyChallenge = generateDailyChallenge(challengeId, category);
    ...
    return {
      ...
      currentCategory: dailyChallenge.category,
  ...
  const resetGame = () => {
    // Generate a truly random challenge ID with high entropy
    // Use timestamp + random number to ensure uniqueness
    const timestamp = Date.now();
    const randomNum = Math.floor(Math.random() * 1000000);
    const randomChallengeId = `new-${timestamp}-${randomNum}`;

    // Explicitly cycle through categories to ensure variety
    const allCategories = ["Animal Kingdom", "Food Fiesta", "Around the World"];
    const currentCategoryIndex = allCategories.indexOf(
      gameState.currentCategory || ""
    );
    const nextCategoryIndex = (currentCategoryIndex + 1) % allCategories.length;
    const nextCategory = allCategories[nextCategoryIndex];

    // Force a new random puzzle with the next category
    ...
    setGameState({
      ...
      currentCategory: newChallenge.category,
    ...
    toast.success(`New ${newChallenge.category} puzzle generated!`, {
      position: "top-center",
    });
  };
  ...
  const useHint = () => {
    ...
    const hintTypes: ("letter" | "word" | "definition")[] = [
      "letter",
      "word",
      "definition",
    ];
    ...
    const randomWord =
      unsolvedWords[Math.floor(Math.random() * unsolvedWords.length)];
    ...
    switch (randomType) {
      ...
      case "definition":
        ...
        hintContent =
          definitions[randomWord] ||
          `A hidden word with ${randomWord.length} letters`;
        break;
    }
